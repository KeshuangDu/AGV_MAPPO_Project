...
class TrainConfig:
    ...
    # ===== 实验标识 =====
    EXPERIMENT_NAME = "h_uni"  # horizontal + unidirectional
    ...
    # ✨ 实验专属目录（与双向区分）
    CHECKPOINT_DIR = './data/checkpoints_h_uni'
    LOG_DIR = './data/logs_h_uni'
    DATA_DIR = './data/generated_data_h_uni'
    TB_LOG_DIR = './runs_h_uni'
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("⚠️  注意：单向模式下AGV只能前进，不能后退")
    print("=" * 60)
    print("启动命令:")
    print(f"  训练: python train_h_uni.py")
    print(f"  监控: tensorboard --logdir={train_config.TB_LOG_DIR}")
    print(f"  评估: python evaluate_h_uni.py")
    print("=" * 60)