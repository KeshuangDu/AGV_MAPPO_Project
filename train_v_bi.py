...
运行方法：
    python train_v_bi.py

TensorBoard监控：
    tensorboard --logdir=./runs_v_bi

✨ 关键配置：
- LAYOUT_TYPE = 'vertical'  (垂直布局)
- BIDIRECTIONAL = True      (双向路由)
"""
...
class Trainer:
    ...
    def __init__(self):
        ...
        env_config.BIDIRECTIONAL = True        # ✨ 双向路由

        print(f"\n{'='*60}")
        print(f"🚀 实验3：垂直布局 + 双向路由 (v_bi)")
        print(f"{'='*60}")
    ...
    def train_episode(self, episode):
        ...
        processed_trajectory = {
            ...
            'advantages': advantages.reshape(-1),  # ✅ 添加advantages
            'returns': returns.reshape(-1)          # ✅ 添加returns
        }
