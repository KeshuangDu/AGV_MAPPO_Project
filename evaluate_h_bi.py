"""
实验1评估脚本：水平布局 + 双向路由
Horizontal Layout + Bidirectional Routing - Evaluation

运行方法：
    python evaluate_h_bi.py --checkpoint ./data/checkpoints_h_bi/mappo_final_1000ep.pt --episodes 100
"""
...
from config.train_config_h_bi import train_config  # ✨ 使用水平双向配置
...
class Evaluator:
    ...
    def __init__(self, checkpoint_path: str):
        ...
        print(f"✅ Evaluation Mode: 水平布局 + 双向路由 (h_bi)")
    ...
    def print_statistics(self):
        ...
        backward = self.eval_results['backward_usage']
        if tasks and backward:
            print(f"\n{'=' * 60}")
            print(f"⭐ 关键指标 - 双向路由优势")
            print(f"{'=' * 60}")
            print(f"  平均任务完成数: {np.mean(tasks):.2f}")
            print(f"  平均后退使用率: {np.mean(backward):.1f}%")  # ✨ 双向特有
    ...
    def visualize_results(self):
        ...
        fig.suptitle('Evaluation Results - 水平双向 (h_bi)',
        ...
        ax.set_title('Backward Usage Rate (Bidirectional Feature)')
        ax.legend()
        ax.grid(True, alpha=0.3)
        ...
        save_path = os.path.join(train_config.LOG_DIR, 'evaluation_results_h_bi.png')
    ...
    def save_results(self):
        ...
        save_path = os.path.join(train_config.LOG_DIR, 'evaluation_results_h_bi.json')
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Evaluate h_bi Model')
    parser.add_argument(
        '--checkpoint',
        type=str,
        default='./data/checkpoints_h_bi/mappo_final_1000ep.pt',
