"""
垂直布局 + 双向路由 评估脚本 v2.0
Vertical Layout + Bidirectional Routing Evaluation with Task Time Tracking
...
运行方法：
    python evaluate_v_bi_v2.py --checkpoint ./data/checkpoints_v_bi/mappo_episode_100.pt --episodes 50
"""
...
class EvaluatorV2:
    ...
    def __init__(self, checkpoint_path: str, experiment_name: str = 'v_bi'):
        ...
        print(f"\n{'='*60}")
        print(f"📊 评估实验 v2.0: 垂直布局 + 双向路由 (v_bi)")
        print(f"✨ 新增: 任务完成时间追踪")
        print(f"{'='*60}")
    ...
    def plot_results(self):
        ...
        axes[4].axvline(np.mean(self.eval_results['backward_usage'])*100,
                       color='red', linestyle='--', label='Mean')
        axes[4].legend()

        # ✨ 任务完成时间分布
        if self.eval_results['task_completion_times']:
            axes[5].hist(self.eval_results['task_completion_times'], bins=20,
                        edgecolor='black', color='green', alpha=0.7)
            axes[5].set_title('Task Completion Time Distribution')
            axes[5].set_xlabel('Time (seconds)')
            axes[5].set_ylabel('Frequency')
            axes[5].axvline(np.mean(self.eval_results['task_completion_times']),
                           color='red', linestyle='--', label='Mean')
            axes[5].legend()
        else:
            axes[5].text(0.5, 0.5, 'No tasks completed',
...
def main():
    parser = argparse.ArgumentParser(description='评估垂直布局+双向路由模型 (v2)')
    ...
    evaluator = EvaluatorV2(checkpoint_path=args.checkpoint, experiment_name='v_bi')
